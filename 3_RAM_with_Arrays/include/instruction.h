...
public:
	 ...
	 */
	Instruction(const string& operand, const OperandType& type, bool array_behaviour, unsigned index)
		:	operand_(operand), type_(type), array_behaviour_(array_behaviour), index_(index) {}
	 ...
	 */
	virtual void Execute(unsigned& program_counter, vector<vector<int>>& data_memory, const vector<pair<string, unsigned>>& labels,
