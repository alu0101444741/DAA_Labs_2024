...
Solution GvnsPMSP::Solve_Modificacion() {  
  ...
  GraspPMSP grasp(grasp_iterations, candidate_list_size, true, 0); // Reinserción en misma máquina en índice 0
  ...
  for (unsigned iteration = 0; iteration < maximum_iterations_; ++iteration) {
    ...
    for (unsigned k = 0; k < k_maximum_; ++k) {     
      ...
      if (vnd_solution.GetTotalCompletionTime() < current_solution.GetTotalCompletionTime()) { 
        ...
        k = 0; // Continuar la búsqueda con la primera estructura de entorno
      }
    }
  }
...
Solution GvnsPMSP::VND_Modificacion(const Solution& solution) { 
  ...
  for (unsigned l = 2; l < local_search_types_; ++l) { // Búsquedas entre máquinas (INTER) en índices l = {2, 3}
